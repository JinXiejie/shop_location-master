...
for mall in malls['mall_id']:
    ...
    sparse_matrix = wifi_infos_sparse_matrix_train.unstack(level = -1)

    wifi_infos_sparse_matrix_train.reset_index(inplace=True)
    # wifi_infos_sparse_matrix_train.drop(['index'], axis=1, inplace=True)

    dataset_in_this_mall = pd.concat((dataset_for_train, sparse_matrix), axis=1)

    test_df = dataset_in_this_mall

    data_for_evaluate = test_df[test_df['time_stamp'] == 0]

    data_for_train = test_df[test_df['time_stamp'] > 0]

    data_for_train_x = data_for_train.drop(['time_stamp', 'shop_label', 'mall_id', 'wifi_infos', 'wifi_infos_raw'],
                                           axis=1)
    data_for_train_y = data_for_train[['shop_label']].astype('int')

    data_for_evaluate_x = data_for_evaluate.drop(['time_stamp', 'shop_label', 'mall_id', 'wifi_infos', 'wifi_infos_raw'],
                                               axis=1)
