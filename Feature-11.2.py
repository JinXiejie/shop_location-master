...
for mall in malls['mall_id']:
    ...
    train_and_test = pd.concat((dataset_for_train, predict_data), axis=0)
    train_and_test.reset_index(inplace=True)
    train_and_test.drop(['index'], axis=1, inplace=True)

    # build sparse matrix for wifi_infos
    wifi_infos_sparse_matrix = train_and_test['wifi_infos'].str.split(";", expand=True).stack().reset_index()
    wifi_infos_sparse_matrix.rename(columns={'level_0': 'record_id', 'level_1': 'drop_id', 0: 'new_wifi_infos'},
                                    inplace=True)
    wifi_infos_sparse_matrix['wifi_id'] = wifi_infos_sparse_matrix['new_wifi_infos'].apply(lambda x: x.split('|')[0])
    wifi_infos_sparse_matrix['signal'] = wifi_infos_sparse_matrix['new_wifi_infos'].apply(lambda x: int(x.split('|')[1]))
    wifi_infos_sparse_matrix.drop(['drop_id', 'new_wifi_infos'], axis=1, inplace=True)
    wifi_infos_sparse_matrix = wifi_infos_sparse_matrix.groupby(['record_id', 'wifi_id'])['signal'].agg(
        'mean').reset_index()
    wifi_infos_sparse_matrix.set_index(['record_id', 'wifi_id'], inplace=True)
    sparse_matrix_train = wifi_infos_sparse_matrix.unstack(level=-1)
    ...
    train_df = pd.concat([dataset_for_train, sparse_matrix_train[0:dataset_for_train.shape[0]]], axis=1)

    test_df = pd.concat((predict_data, sparse_matrix_train[dataset_for_train.shape[0]:train_and_test.shape[0]]), axis=1)

    # test_df = wifi_infos_sparse_matrix_train

    # release the memory
    wifi_infos_sparse_matrix = None
    train_and_test = None
    sparse_matrix_train = None

    data_for_evaluate = train_df[train_df['time_stamp'] == 0]

    data_for_train = train_df[train_df['time_stamp'] > 0]
    ...
    # test_dataset_in_this_mall = pd.concat((predict_data, sparse_matrix_train[dataset_for_train.shape[0]:train_and_test.shape[0]]), axis=1)
